...
@interface UniWebView : IMYWebView// UIWebView
...
@interface UniWebViewManager()<IMYWebViewDelegate> {
...
-(void) webViewNameCleanCache:(NSString *)name {
    UniWebView *webView = [_webViewDic objectForKey:name];
    [[NSURLCache sharedURLCache] removeCachedResponseForRequest:webView.originRequest];

    [[NSURLCache sharedURLCache] removeAllCachedResponses];
    NSDate* dateFrom = [NSDate dateWithTimeIntervalSince1970:0];
    NSSet* webSiteDataType = [WKWebsiteDataStore allWebsiteDataTypes];
    [[WKWebsiteDataStore defaultDataStore] removeDataOfTypes:webSiteDataType modifiedSince:dateFrom completionHandler:^{
    }];
}
...
-(void) performJavaScript:(NSDictionary *)info {
    ...
    [webView evaluateJavaScript:js completionHandler:^(id, NSError *) {}];
//    NSString *result = [webView stringByEvaluatingJavaScriptFromString:js];
//    if (callback) {
//        UnitySendMessage([webViewName UTF8String], "EvalJavaScriptFinished", [result UTF8String]);
//    }
}
...
- (void)webViewDidFinishLoad:(UniWebView *)webView {
    ...
    webView.currentUrl = webView.originRequest.mainDocumentURL.absoluteString;
    UnitySendMessage([webViewName UTF8String], "LoadWebViewComplete", "");
}
...
- (void)webView:(UniWebView *)webView didFailLoadWithError:(NSError *)error {
    ...
    webView.currentUrl = webView.originRequest.mainDocumentURL.absoluteString;
    UnitySendMessage([webViewName UTF8String], "LoadWebViewComplete", [error.localizedDescription UTF8String]);
}
