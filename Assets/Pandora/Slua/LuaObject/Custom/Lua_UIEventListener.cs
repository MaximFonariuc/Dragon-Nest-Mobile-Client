...
public class Lua_UIEventListener : LuaObject {
	...
	[MonoPInvokeCallbackAttribute(typeof(LuaCSFunction))]
	static public int set_onLongPress(IntPtr l) {
		try {
			UIEventListener self=(UIEventListener)checkSelf(l);
			UIEventListener.VoidDelegate v;
			int op=LuaDelegation.checkDelegate(l,2,out v);
			if(op==0) self.onLongPress=v;
			else if(op==1) self.onLongPress+=v;
			else if(op==2) self.onLongPress-=v;
			pushValue(l,true);
			return 1;
		}
		catch(Exception e) {
	...
	static public void reg(IntPtr l) {
		...
		addMember(l,"onLongPress",null,set_onLongPress,true);
