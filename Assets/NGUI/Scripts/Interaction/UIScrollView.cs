...
public class UIScrollView : MonoBehaviour
{
    ...
    public float moveControllerTime = 0;
	...
	public bool RestrictWithinBounds (bool instant, bool horizontal, bool vertical)
	{
		...
		if (constraint.sqrMagnitude > 1f)
		{
			if (!instant && dragEffect == DragEffect.MomentumAndSpring)
			{
                ...
                sp = SpringPanel.Begin(mPanel.gameObject, sp, pos, 13f);
            }
            else
			{
    ...
    public void ResetPosition(float pos)
    {
        if (NGUITools.GetActive(this))
        {
            ...
            Vector2 pv = NGUIMath.GetPivotOffset(contentPivot);
            // First move the position back to where it would be if the scroll bars got reset to zero
            SetDragAmount(pv.x, pos, false);
            // Next move the clipping area back and update the scroll bars
            SetDragAmount(pv.x, pos, true);
        }
    }
	...
	public void Drag ()
	{
        if (enabled && NGUITools.GetActive(gameObject) && mShouldMove && Time.time > moveControllerTime)
		{
	...
	void LateUpdate ()
	{
        ...
        if (mAutoMove)
        {
            if (mMoveAmount*mMoveSpeed <= mDestAmount*mMoveSpeed)
            {
                if (movement == Movement.Custom)
                {
                    if (customMovement.x > 0)
                    {
                        mMoveAmount += delta * mMoveSpeed;
                        SetDragAmount(mMoveAmount, 0, false);
                    }
                    else if (customMovement.y > 0)
                    {
                        mMoveAmount += delta * mMoveSpeed;
                        SetDragAmount(0, mMoveAmount, false);
                    }
                }
