...
static public class NGUIMath
{
	...
	static public Rect ConvertToTexCoords (Rect rect, int width, int height, float offset = 0.0f)
	{
		Rect final = rect;

        if (width != 0f && height != 0f)
        {
            final.xMin = (rect.xMin + offset) / width;
            final.xMax = (rect.xMax - offset) / width;
            final.yMin = 1f - (rect.yMax - offset) / height;
            final.yMax = 1f - (rect.yMin + offset) / height;
        }
        return final;
    }
