...
static public class NGUIText
{
    ...
    static public Vector2 CalculatePrintedSize(string text, int actualRectWidth = -1)
	{
        if (actualRectWidth < 0)
            actualRectWidth = rectWidth;
			...
			for (int i = 0; i < textLength; ++i)
			{
				...
				if (symbol == null)
				{
					...
					if (w != 0f)
					{
                        ...
                        if (Mathf.RoundToInt(x + w) > actualRectWidth)
						{
    ...
    static public bool WrapText(string text, out string finalText, bool keepCharCount)
	{
        ...
        List<int> cacheinvalid = new List<int>();
		// Run through all characters
		for (; offset < textLength; ++offset)
		{
			char  ch = text[offset];
            if (ch > 12287) eastern = true;
            else if (ch == ' ') eastern = false;
            ...
            if (encoding && ParseSymbol(text, ref offset)) { --offset; prev = text[offset]; continue; }
			...
			if (Mathf.RoundToInt(remainingWidth) < 0)
			{
				...
				else
				{
                    ...
                    else
                    {
                        ...
                        sb.Append(text, start, end);
                        if (!keepCharCount) EndLine(ref sb);
                        start = offset;
                        continue;
                    }
				}
        ...
        if(hasInvalidString)
        {
            ...
            {
                if (cacheinvalid.Contains(i))
                    bb.Append("*");
                else
                    bb.Append(text[i]);
            }
            finalText = bb.ToString();
        }
        else
        {
		...
		for (int i = 0; i < textLength; ++i)
		{
			...
			else // No symbol present
			{
				...
				if (underline || strikethrough)
				{
                    ...
                    float scaleheight = Mathf.Abs(v0y - v1y)*0.25f;
                    v0y += scaleheight;
                    v1y -= scaleheight;
