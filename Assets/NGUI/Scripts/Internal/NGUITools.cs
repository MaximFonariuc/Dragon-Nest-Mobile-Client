...
static public class NGUITools
{
    ...
    static XFmod mUIFmod;

	static bool mLoaded = false;
	static float mGlobalVolume = 1f;
    static int UILayer = 5;
    ...
    static public void PlayFmod(string eventPath)
    {
        ...
        if (mUIFmod == null)
        {
            if (GameObject.Find("UIRoot/uiplayaudio") != null)
                mUIFmod = GameObject.Find("UIRoot/uiplayaudio").GetComponent<XFmod>();
            else
                mUIFmod = GameObject.Find("UIRoot(Clone)/uiplayaudio").GetComponent<XFmod>();
        }

        if (mUIFmod == null) return;

        mUIFmod.StartEvent(eventPath, XUtliPoolLib.AudioChannel.SkillCombine);
#endif
    }
    ...
    static public BoxCollider AddWidgetCollider(GameObject go,UIWidget w, bool considerInactive)
    {
        if (go != null)
        {
            Collider col = go.GetComponent<Collider>();
            ...
            if (w != null) w.autoResizeBoxCollider = true;
            UpdateWidgetCollider(w, box, considerInactive);
            return box;
        }
        return null;
    }
	...
	static public void UpdateWidgetCollider (UIWidget w,BoxCollider box, bool considerInactive)
	{
		if (box != null)
		{
	...
	static public void AttachChild(GameObject parent,GameObject go)
	{
		if (parent != null) 
		{
			Transform t = go.transform;
    ...
    static public void SetActive(GameObject go, bool state, bool createPanel) { SetActive(go, state, true, createPanel); }
    ...
    static public void SetActive(GameObject go, bool state, bool compatibilityMode, bool createPanel)
    {
        if (go)
        {
            if (state)
    ...
    static List<UIRect> tmpRects = new List<UIRect>();
	static public void MarkParentAsChanged (GameObject go)
	{
        tmpRects.Clear();
        go.GetComponentsInChildren<UIRect>(tmpRects);
        if(tmpRects.Count>0)
        {
            UIRect rect = tmpRects[0];
            if (rect.gameObject == go)
            {
                //set parent UIRect
                rect.ChangeParent();
                UIPanel p = UIPanel.Find(rect.cachedTransform, true, rect.cachedGameObject.layer);
                for (int i = 0, imax = tmpRects.Count; i < imax; ++i)
                    tmpRects[i].SetPanel(p);
            }
            else
            {
#if UNITY_EDITOR
                Debug.LogWarning("GameObject no UIRect:" + go.name);
#endif
                for (int i = 0, imax = tmpRects.Count; i < imax; ++i)
                    tmpRects[i].ParentHasChanged();
            }
        }
        tmpRects.Clear();
	}
    static public void ParentPanelChanged(GameObject go, UIPanel panel)
    {
        if (panel!=null)
        {
            tmpRects.Clear();
    ...
    static public void ParentPanelChanged(GameObject go, UIRect parent, UIPanel panel)
    {
        if (panel != null)
        {
            tmpRects.Clear();
