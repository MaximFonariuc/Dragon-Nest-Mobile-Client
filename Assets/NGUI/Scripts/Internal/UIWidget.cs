...
//public interface ITextureCompare
//{
//    bool Compare(ITextureCompare src);
//    Texture mainTexture { get; }
//    Texture mainTexture1 { get; }
//}
...
public class UIWidget : UIRect
{
    ...
    public class Decorate
    {
        ...
        [SerializeField]
        public UIWidget renderSort = null;
        [System.NonSerialized]
        public GameObject gameObject = null;
        [System.NonSerialized]
        public GameObject tempObject = null;

        public static List<ControlParticle> controls = new List<ControlParticle>();
        public void RenderSort()
        {
            if (gameObject == null) return;
            controls.Clear();
            gameObject.GetComponentsInChildren<ControlParticle>(controls);
            if (controls.Count == 0) return;
            for (int i = 0,length = controls.Count; i < length; i++)
            {
                if (renderSort == null)
                {
                    controls[i].autoFindWidget = true;
                }
                else
                {
                    controls[i].autoFindWidget = false;
                    controls[i].widget = renderSort;
                }
            }
            controls.Clear();
        }
    ...
    [HideInInspector]public uint uid = 1;
    [HideInInspector]public string sid;
    [HideInInspector][SerializeField]public List<Decorate> decorateList;
    ...
    /// <summary>
    /// Modify by rongxutao, If find UIPanel automatically
    /// </summary>
    ...
    public bool calcRenderQueue = false;
    //public bool callRegisterCb = false;
	...
	[HideInInspector][System.NonSerialized] public int textureIndex;

	protected Vector3[] mCorners = new Vector3[4];

    public void SetupDecorate()
    {
        if (!Application.isPlaying) return;
        if (decorateList == null || decorateList.Count == 0) return;
        for(int i = 0,length = decorateList.Count;i < length; i++)
        {
            Decorate decorate = decorateList[i];
            if (!string.IsNullOrEmpty(decorate.decoratePath))
            {
                if(decorate.gameObject == null)
                {
                    UnityEngine.Object obj = XResourceLoaderMgr.singleton.CreateFromPrefab(decorate.decoratePath);
                    if (obj != null) decorate.gameObject = obj as GameObject;
                }
                if (decorate.gameObject == null)
                {
                    XDebug.singleton.AddErrorLog(" Not Decorate GameObject", decorate.decoratePath);
                    continue;
                }
                NGUITools.AttachChild(gameObject, decorate.gameObject);
                Transform t = decorate.gameObject.transform;
                t.localPosition = decorate.offset;
                t.localScale = decorate.scale;
                Quaternion q = t.localRotation;
                q.eulerAngles = decorate.rotation;
                t.localRotation = q;
                decorate.RenderSort();
    ...
    private void DestroyDecorate(GameObject go)
    {
        if (!Application.isPlaying) return;
        if (go != null)
        {
	...
	public bool isVisible { get { return mIsVisibleByPanel && mIsVisibleByAlpha && mIsInFront && finalAlpha > 0.001f && NGUITools.GetActive(this) && !isFaraway; } }


   public const int faraway = 2016;

    private bool isFaraway { get { return cachedTransform.localPosition.x == faraway || cachedTransform.localPosition.y == faraway; } }
    ...
    //public virtual bool Compare(ITextureCompare src)
    //{
    //    return mainTexture == src.mainTexture && mainTexture1 == src.mainTexture1;
    //}
	...
	{
        if (NGUITools.GetActive(this) && boxColliderCache!=null)
		{
            NGUITools.UpdateWidgetCollider(this, boxColliderCache, true);
		}
	}
	...
	{
		mGo = gameObject;
        //if (boxColliderCache == null)
        //    boxColliderCache = collider as BoxCollider;
...
//#endif
        if (storePanel == null)
        {
            storePanel = UIPanel.Find(cachedTransform, false, cachedGameObject.layer);
        }
		base.OnInit();
		RemoveFromPanel();
		mMoved = true;
...
//        }
	...
	{
        if (panel == null && autoFindPanel) CreatePanel();
#if UNITY_EDITOR
    ...
    protected virtual void OnDestroy() {
        ...
        if(decorateList != null)
        {
            for(int i = 0,length = decorateList.Count;i < length; i++)
            {
                if(decorateList[i].gameObject != null)
                    DestroyDecorate(decorateList[i].gameObject);
            }
        }
        geometry.Clear();
        RemoveFromPanel();
    ...
    {
        if (geometry.WriteToBuffers(v, u, c, n, t))
        {
            XUtliPoolLib.XDebug.singleton.AddErrorLog2("Too many vertex panel:{0}", this.name);
        }
    }
