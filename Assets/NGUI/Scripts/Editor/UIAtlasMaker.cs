		...
		else
		{
			...
			if (create)
			{
				...
				if (!string.IsNullOrEmpty(prefabPath))
				{
					...
					if (mat == null)
					{
						Shader shader = Shader.Find(NGUISettings.atlasPMA ? "Unlit/Premultiplied Colored" : "Custom/UI/RGBA");
		...
		if (spriteList.Count > 0)
		{
			NGUIEditorTools.DrawHeader("Sprites", true);
			{
				...
				if (delete)
				{
					...
					UpdateAtlas(NGUISettings.atlas, sprites);
                    //XEditor.AssetModify.PostImportUIAtlas(NGUISettings.atlas.texture as Texture2D);
					mDelNames.Clear();
