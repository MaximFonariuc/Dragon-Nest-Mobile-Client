...
public class UIWidgetInspector : UIRectEditor
{
    ...
    {
        public UIWidget.Decorate decorate;
    ...
    private Dictionary<string, GameObject> decorateDic;
    private void DrawDecorate()
    {
        GUILayout.Label("Decorate", GUILayout.ExpandWidth(false));
        EditorGUILayout.BeginHorizontal();

        if (GUILayout.Button("Add"))
        {
            if (mWidget.decorateList == null) mWidget.decorateList = new List<UIWidget.Decorate>();
            mWidget.decorateList.Add(new UIWidget.Decorate());
        }

        EditorGUILayout.EndHorizontal();
        if (mWidget.decorateList != null)
        {
            GUILayout.Label("Decorate Size:   " + mWidget.decorateList.Count.ToString());
            for (int i = 0, length = mWidget.decorateList.Count; i < length; i++)
            {

                UIWidget.Decorate decorate = mWidget.decorateList[i];
                GUILayout.Label("----------------------", GUILayout.ExpandWidth(false));
                EditorGUILayout.BeginHorizontal();
                if (GUILayout.Button("x"))
                {
                    mWidget.decorateList[i].tempObject = null;
                    mWidget.decorateList.RemoveAt(i);
                    break;
                }
                decorate.tempObject = EditorGUILayout.ObjectField(decorate.tempObject, typeof(GameObject), true) as GameObject;
                decorate.decoratePath = GetPath(decorate.tempObject, ".prefab");
                EditorGUILayout.EndHorizontal();
                GUILayout.Label("URL :" + decorate.decoratePath);
                decorate.offset = EditorGUILayout.Vector3Field("offset :", decorate.offset);
                decorate.rotation = EditorGUILayout.Vector3Field("rotation:", decorate.rotation);
                decorate.scale = EditorGUILayout.Vector3Field("scale:", decorate.scale);
                EditorGUILayout.BeginHorizontal();
                decorate.renderSort = EditorGUILayout.ObjectField(decorate.renderSort, typeof(UIWidget), true) as UIWidget;
    ...
    protected override void OnEnable ()
	{
		...
		instance = this;
		mWidget = target as UIWidget;
        if (mWidget.decorateList != null && mWidget.decorateList.Count > 0)
        {
            for (int i = 0, length = mWidget.decorateList.Count; i < length; i++)
            {
                if (!string.IsNullOrEmpty(mWidget.decorateList[i].decoratePath) && mWidget.decorateList[i].tempObject == null)
                {
                        mWidget.decorateList[i].tempObject = AssetDatabase.LoadAssetAtPath("Assets/Resources/" + mWidget.decorateList[i].decoratePath + ".prefab", typeof(GameObject)) as GameObject;
                }
            }
        }         
    }
	...
	protected override void DrawCustomProperties ()
	{
		...
		if (NGUIEditorTools.DrawHeader("Widget"))
		{
            ...
            GUILayout.BeginHorizontal();
            NGUIEditorTools.DrawProperty("AutoFindPanel", serializedObject, "autoFindPanel", GUILayout.Width(100f));
            GUILayout.EndHorizontal();
            ...
            GUILayout.Label(mWidget.panel != null ? mWidget.panel.name : "null");
            GUILayout.EndHorizontal();
            ...
            if(mWidget.drawCall!=null)
            {
                GUILayout.BeginHorizontal();
                GUILayout.Label("Mat");
                EditorGUILayout.ObjectField("Material", mWidget.drawCall.dynamicMaterial, typeof(Material), false);
                GUILayout.EndHorizontal();
            }
