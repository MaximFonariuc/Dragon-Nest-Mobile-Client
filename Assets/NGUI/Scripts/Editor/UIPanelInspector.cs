...
public class UIPanelInspector : UIRectEditor
{
    ...
    private List<UIWidget> decorateList;
    private void FindDecorateList()
    {
        if (decorateList == null) decorateList = new List<UIWidget>();
        decorateList.Clear();
        UIWidget[] widgets = mPanel.GetComponentsInChildren<UIWidget>(true);
        for(int i = 0, length = widgets.Length;i < length ; i++)
        {
            if (widgets[i].decorateList == null || widgets[i].decorateList.Count == 0) continue;
            for(int j = 0,size = widgets[i].decorateList.Count; j < size; j++)
            {
                UIWidget.Decorate decorate = widgets[i].decorateList[j];
                if(!string.IsNullOrEmpty(decorate.decoratePath)&& decorate.tempObject == null)
                    decorate.tempObject = AssetDatabase.LoadAssetAtPath("Assets/Resources/" + decorate.decoratePath + ".prefab", typeof(GameObject)) as GameObject;

            }
            decorateList.Add(widgets[i]);
        }
    }

    private  void DrawDecorateList()
    {
        
        GUILayout.BeginHorizontal();
      
        GUILayout.Label("Decorate List :" + (decorateList== null? 0:decorateList.Count));
        ...
        if (decorateList != null) {
            for (int i = 0, length = decorateList.Count;i< length; i++)
            {
                UIWidget info = decorateList[i];
                EditorGUILayout.BeginHorizontal();
                GUILayout.Label("widget:"+i.ToString());
                EditorGUILayout.ObjectField(info, typeof(GameObject), false);
                EditorGUILayout.EndHorizontal();
                if (info.decorateList == null || info.decorateList.Count == 0) continue;
                for(int j = 0,size = info.decorateList.Count;j < size; j++)
                {
                    UIWidget.Decorate decorate = info.decorateList[j];
                    GUILayout.BeginHorizontal();
                    if (decorate.tempObject != null)
                    {
                        GUILayout.Label(decorate.tempObject.name);
                    }
                    else
                    {
                        GUILayout.Label("Resource None");
                    }                    
                    EditorGUILayout.ObjectField(decorate.tempObject, typeof(GameObject), true);
                    GUILayout.EndHorizontal();
                }
	...
	{
		bool useMerge = EditorGUILayout.Toggle ("UseMerge", mPanel.useMerge);

		if (useMerge != mPanel.useMerge) {
			mPanel.useMerge = useMerge;
		}

		int updatePreFrame = EditorGUILayout.IntField ("UpdatePreFrame", mPanel.updatePreFrame);
		if (updatePreFrame != mPanel.updatePreFrame)
			mPanel.updatePreFrame = updatePreFrame;

		int updateFrame = EditorGUILayout.IntField ("UpdatePrameIndex", mPanel.updateFrameIndex);
		if (updateFrame != mPanel.updateFrameIndex)
			mPanel.updateFrameIndex = updateFrame;
        ...
        if (mPanel.clipping != UIDrawCall.Clipping.None)
        {
            Vector4 clipRange = mPanel.finalClipRegion;
            Vector3 panelPos = mPanel.root.transform.InverseTransformPoint(mPanel.transform.position);
            ...
            GUILayout.BeginHorizontal();
            GUILayout.Space(80f);
            GUILayout.Label("Screen Project Size");
            
            //Debug.Log("PanelPos:" + panelPos.ToString());
            EditorGUILayout.LabelField("", string.Format("Left:{0} Down:{1} Right:{2} Top:{3}", left, down, right, top));
            GUILayout.EndHorizontal();
        }
		...
		if (NGUIEditorTools.DrawHeader("Advanced Options"))
		{
            ...
            GUILayout.BeginHorizontal();
            bool vertexCount = EditorGUILayout.Toggle("Vertex", mPanel.showVertexCount, GUILayout.Width(100f));
            GUILayout.Label("Vertex Count:" + mPanel.vertexCount.ToString());
            GUILayout.EndHorizontal();
            if (mPanel.showVertexCount != vertexCount)
            {
                mPanel.showVertexCount = !mPanel.showVertexCount;
                EditorUtility.SetDirty(mPanel);
                EditorWindow.FocusWindowIfItsOpen<UIPanelTool>();
            }

            GUILayout.BeginHorizontal();
            bool needUpdate = EditorGUILayout.Toggle("Draw", mPanel.NeedUpdate, GUILayout.Width(100f));
            GUILayout.Label("Generate DrawCall");
            GUILayout.EndHorizontal();
            if (mPanel.NeedUpdate != needUpdate)
            {
                mPanel.NeedUpdate = !mPanel.NeedUpdate;
                EditorUtility.SetDirty(mPanel);
                EditorWindow.FocusWindowIfItsOpen<UIPanelTool>();
            }
