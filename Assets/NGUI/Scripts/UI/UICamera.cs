...
public class UICamera : MonoBehaviour
{
    ...
    public GameObject touchFx = null;
	...
	static protected void SetSelection (GameObject go, ControlScheme scheme)
	{
		...
		else if (mCurrentSelection != go)
		{
			...
			if (UICamera.list.size > 0)
			{
                ...
                if (cam != null) cam.mChangeSelection = true;// cam.StartCoroutine(cam.ChangeSelection());
			}
		}
	}
    ...
    static public void Notify(GameObject go, string funcName, object obj)
    {
        ...
        if (NGUITools.GetActive(go))
        {
            ...
            {
                clickpath = FindPath(go);
                if (HotfixManager.Instance.TryFixClick(XUtliPoolLib.HotfixMode.BEFORE, clickpath))
                {
                    mNotifying = false;
                    return;
                }
            ...
            if (genericEventHandler != null && genericEventHandler != go)
            {
                ...
                if (funcName.Equals("OnClick"))
                {
                    HotfixManager.Instance.TryFixClick(XUtliPoolLib.HotfixMode.AFTER, clickpath);
                }
            }
        }
        mNotifying = false;
    ...
    public static string FindPath(GameObject go)
    {
        ...
        while (tran.parent != null && !tran.parent.name.StartsWith("UIRoot"))
        {
            tran = tran.parent;
            path = tran.name + "/" + path;
        }
...
//#endif
...
#else
		if (!handlesEvents||!NGUITools.mEnableLoadingUpdate) return;
#endif
        current = this;
	...
	public void ProcessMouse ()
	{
        ...
        else if (!justPressed && isPressed && posChanged)
        {
            float dist = Mathf.Pow(Input.mousePosition.x - lastClickPosition.x, 2) + Mathf.Pow(Input.mousePosition.y - lastClickPosition.y, 2);

            if (dist > touchDragThreshold * touchDragThreshold)
            {
                if (UIPointTrail.current != null)
                {
                    Vector3 p = mMouse[0].pos;
                    p.x -= (mWidth >> 1);
                    p.y -= (mHeight >> 1);
                    UIPointTrail.current.AddPoint(p, RealTime.time);
                }
        ...
        else if (justReleased)
        {
            ...
            // mouse up at same point as down
            //else if (Mathf.Abs(Input.mousePosition.x - lastClickPosition.x) <= float.Epsilon
            //    && Mathf.Abs(Input.mousePosition.y - lastClickPosition.y) <= float.Epsilon)
            else
            {
                float dist = Mathf.Pow(Input.mousePosition.x - lastClickPosition.x, 2) + Mathf.Pow(Input.mousePosition.y - lastClickPosition.y, 2);
                ...
                if (dist < touchDragThreshold * touchDragThreshold)
                {
                    if (touchFx != null)
                    {
                        touchFx.SetActive(true);

                        float f = mRoot.GetPixelSizeAdjustment(mHeight);

                        touchFx.transform.localPosition =
                            new Vector3(Input.mousePosition.x - mWidth/2, Input.mousePosition.y - mHeight/2, 0.0f)*f;

                        UISpriteAnimation sa = touchFx.GetComponent<UISpriteAnimation>();
                        sa.Reset();
                    }
                }
            }
	...
	public void ProcessTouches ()
	{
		...
		for (int i = 0; i < Input.touchCount; ++i)
		{
            ...
            {
                float dist = Mathf.Pow(touch.position.x - lastClickPosition.x, 2) + Mathf.Pow(touch.position.y - lastClickPosition.y, 2);
            ...
            else if ((touch.phase == TouchPhase.Canceled) || (touch.phase == TouchPhase.Ended))
            {
                ...
                // mouse up at same point as down
                //else if (lastClickPosition == touch.position)
                else
                {
                    float dist = Mathf.Pow(Input.mousePosition.x - lastClickPosition.x, 2) + Mathf.Pow(Input.mousePosition.y - lastClickPosition.y, 2);
