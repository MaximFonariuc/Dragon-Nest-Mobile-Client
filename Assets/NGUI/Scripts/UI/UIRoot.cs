...
#endif
        float swbh = Screen.height / (float)Screen.width;
        float bwbh = base_ui_height /(float) base_ui_width;

        if (swbh < bwbh)
            manualHeight = base_ui_height;
        else
            manualHeight = Screen.height * base_ui_width / Screen.width;
