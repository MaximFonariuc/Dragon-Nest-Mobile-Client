...
public class UILabel : UIWidget
{
    ...
    [HideInInspector][SerializeField] bool mOutLineLock = false;
	...
	protected void SetActiveFont (Font fnt)
	{
		if (mActiveTTF != fnt)
		{
			if (mActiveTTF != null)
			{
                ...
                else Font.textureRebuilt -= OnFontTextureChanged;
			}
    ...
    {
        mPrintedSize = Mathf.Abs(defaultFontSize);
    }
    ...
    public void SetFont(string path, string suffix = ".ttf")
    {
        if (fontPath != path)
        {
            DestroyTureTypeFont(trueTypeFont);
            DestroyUIFont(bitmapFont);
            ...
            else
            {
                if (suffix == ".ttf")
                {
                    Font font = XResourceLoaderMgr.singleton.GetSharedResource<Font>(path, suffix);
                    trueTypeFont = font;
                }
                else if (suffix == ".prefab")
                {
                    GameObject obj = XResourceLoaderMgr.singleton.GetSharedResource<GameObject>(path, ".prefab");
                    bitmapFont = obj == null ? null : obj.GetComponent<UIFont>();
                }
            }
    ...
    public void DestroyTureTypeFont(Font font)
    {
        if (font != null)
        {
            //XResourceLoaderMgr.singleton.UnSafeDestroyShareResource(fontPath, fontSuffix, font);
        }
    }

    public void DestroyUIFont(UIFont font)
    {
        if (font != null)
        {
            //XResourceLoaderMgr.singleton.UnSafeDestroyShareResource(fontPath, fontSuffix, font.gameObject);
        }
    }
