...
public class LevelWave : ScriptableObject
{
    ...
    [SerializeField] public LevelSpawnType _spawnType;

	[SerializeField] public float _time;

	[SerializeField] public int _loopInterval;

    [SerializeField] public uint _enemyid;
    [SerializeField] public int _randomid;
    [SerializeField] public string _script;
    [SerializeField] public bool _repeat;

    [SerializeField] public GameObject _prefab;

    [SerializeField] public int _yRotate;

    [SerializeField] public string _preWaves;
    [SerializeField] public int _preWavePercent;
    [SerializeField] public string _doodad_id;
    [SerializeField] public int _doodad_percent;

    [SerializeField] public string _exString;

	[SerializeField] public List<int> _prefabSlot;

	[SerializeField] public bool _bVisibleInEditor = true;

    [SerializeField] public float _roundRidus;

    [SerializeField] public int _roundCount;
        ...
        set
        {
            if (_enemyid != value)
            {
                if (value > 0)
                {
                    ...
                    {
                        XEntityStatistics.RowData npcInfo = LevelMgr.EnemyList.GetByID(_enemyid);

                        if (npcInfo == null) return;

                        _prefab = Resources.Load("Prefabs/" + XAnimationLibrary.AssociatedAnimations(npcInfo.PresentID).Prefab) as GameObject;
                        if (_window != null)
                            _window.RegenerateIcon64();
                    }
    ...
    protected void ParseInfo( string data)
    {
        ...
        switch (type)
        {
            ...
            case InfoType.TypeMonsterInfo:
            {
                ...
                float x = XParse.Parse(strFloats[1]);
                float y = XParse.Parse(strFloats[2]);
                float z = XParse.Parse(strFloats[3]);
                ...
                {
                    rotateY = XParse.Parse(strFloats[4]);
                }
                ...
                if (_prefab != null)
                {
                    ...
                    XEntityStatistics.RowData sData = XStatisticsLibrary.AssociatedData((uint)_enemyid);
                    if (sData == null)
                    {
                    ...
                    {
                        XEntityPresentation.RowData pData = XAnimationLibrary.AssociatedAnimations(sData.PresentID);
                        go.transform.localScale = new Vector3(pData.Scale, pData.Scale,pData.Scale); 
                    }
    ...
    public void SetInstanceFace()
    {
        foreach (int i in _prefabSlot)
        {
            ...
            if (go != null)
            {
                //go.transform.Rotate(new Vector3(0.0f, 1.0f, 0.0f), (float)_yRotate);
	...
	{
		if(_prefabSlot.Count > 0)
		{
			int index = GetMaxSlot();
			string lastPrefabName = GetMonsterName(_id, index);
			GameObject go = GameObject.Find (lastPrefabName);
