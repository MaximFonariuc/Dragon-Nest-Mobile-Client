...
public static class InjectEditor
{
    ...
    private static bool DoCodeInjectorBuild(string plaformTag)
    {
        ...
        string xlib = Application.dataPath + @"/Lib";
        if (!DoCodeInjector(xlib)) return false;
        return true;
    }
    ...
    {
        if (hasMidCodeInjectored || Application.isPlaying || EditorApplication.isPlaying || ABSystem.ABBuilder.isBuildAB) return;
