...
public class ResourceChecker : EditorWindow
{
        ...
        switch (format)
        {
            ...
            */
            ...
            case TextureFormat.BGRA32: // Format returned by iPhone camera.
                return 32;
            /*
            // Устаревшие форматы, удаленные в Unity 5+
    ...
    private bool CheckAlphaChannel(Texture texture)
    {
        if (texture is Texture2D)
        {
            Color32[] colors = (texture as Texture2D).GetPixels32();
    ...
    void ListTextures()
    {
        ...
        foreach (TextureDetails tDetails in ActiveTextures)
        {
            ...
            string sizeLabel = "Size:" + tDetails.texture.width + "x" + tDetails.texture.height;
            if (tDetails.isCubeMap) sizeLabel += "x6";
            sizeLabel += "  mipMap:- " + tDetails.mipMapCount + "  mip";
            sizeLabel += " \n" + FormatSizeString(tDetails.memSizeKB) + " - " + tDetails.format + "";

            GUILayout.Label(sizeLabel, GUILayout.Width(400));
            if (GUILayout.Button(tDetails.FoundInMaterials.Count + " Mat", GUILayout.Width(100)))
            {
                SelectObjects(tDetails.FoundInMaterials, ctrlPressed);
            }

            if (GUILayout.Button(tDetails.FoundInRenderers.Count + " GO", GUILayout.Width(100)))
            {
        ...
        foreach (MaterialDetails tDetails in ActiveMaterials)
        {
            if (tDetails.material != null)
            {
                ...
                if (tDetails.material.mainTexture != null)
                    GUILayout.Box(tDetails.material.mainTexture, GUILayout.Width(ThumbnailWidth),
        ...
        foreach (MeshDetails tDetails in ActiveMeshDetails)
        {
            if (tDetails.mesh != null)
            {
                ...
                */
                ...
                if (GUILayout.Button(tDetails.mesh.name, GUILayout.Width(300)))
                {
                ...
                if (GUILayout.Button(tDetails.FoundInMeshFilters.Count + "  GO", GUILayout.Width(100)))
                {
                    List<Object> FoundObjects = new List<Object>();
                ...
                if (GUILayout.Button(tDetails.FoundInSkinnedMeshRenderer.Count + "  GO", GUILayout.Width(100)))
                {
                    List<Object> FoundObjects = new List<Object>();
    ...
    void CheckResources()
    {
        ...
        Debug.Log(string.Format("renderers.length = {0}", renderers.Length));
        ...
        SkinnedMeshRenderer[] skinnedMeshRenderers =
            (SkinnedMeshRenderer[])FindObjectsOfType(typeof(SkinnedMeshRenderer));
