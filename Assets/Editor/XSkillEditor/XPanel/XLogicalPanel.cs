	...
	public class XLogicalPanel : XPanel
	{
       ...
       // private List<string> _states = new List<string>();
        ...
        protected override void OnInnerGUI()
        {
            ...
            {
                Hoster.SkillData.Logical.QTE_Key = EditorGUILayout.Popup("QTE Key", Hoster.SkillData.Logical.QTE_Key, XQTEStatusLibrary.KeyList.ToArray());
            }
            EditorGUILayout.Space();
            ...
            if (GUILayout.Button(_content_add, GUILayout.MaxWidth(30)))
            {
                ...
                {
                    XDebug.singleton.AddErrorLog("Too much QTE(should < 4 Now)");
                }
            }
            ...
            Hoster.SkillData.Logical.CanReplacedby = EditorGUILayout.MaskField("Can Replaced By", Hoster.SkillData.Logical.CanReplacedby, XSkillPanel.skill_type);
            ...
            EditorGUILayout.Space();
            EditorGUILayout.LabelField("Add Syntonic CoolDown Skill", _myLabelStyle);
            EditorGUILayout.BeginHorizontal();
            EditorGUILayout.LabelField("Skill Name", Hoster.SkillData.Logical.Syntonic_CoolDown_Skill);
            if (Hoster.SkillData.Logical.Syntonic_CoolDown_Skill != null && Hoster.SkillData.Logical.Syntonic_CoolDown_Skill.Length > 0)
            {
                if (GUILayout.Button("Delete", GUILayout.MaxWidth(70)))
                {
                    Hoster.SkillData.Logical.Syntonic_CoolDown_Skill = null;
                }
            }
            if (GUILayout.Button("Browser", GUILayout.MaxWidth(70)))
            {
                ...
                if (file.Length > 0)
                {
                    int s = file.LastIndexOf('/');
                    int e = file.LastIndexOf('.');
                    Hoster.SkillData.Logical.Syntonic_CoolDown_Skill = file.Substring(s + 1, e - s - 1);
                }
            }
            EditorGUILayout.EndHorizontal();
        }
