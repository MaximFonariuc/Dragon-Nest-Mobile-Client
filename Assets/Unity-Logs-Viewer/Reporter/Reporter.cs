...
#if MINIIT_APPLOVIN || MINIIT_IRONSOURCE
#define MINIIT_ADS
#endif
...
public class Images
{
	...
	public Texture2D firebaseImage;
	public Texture2D adsImage;
...
public class Reporter : MonoBehaviour
{
	...
	GUIContent firebaseContent;
	GUIContent adsContent;
	...
	public void Initialize()
	{
		...
		adsContent = new GUIContent("", images.adsImage, "Ads Debug");
		firebaseContent = new GUIContent("", images.firebaseImage, "Firebase Debug");
		...
		isFirebaseDebugActive = PlayerPrefs.GetInt("FirebaseDebugEnabled", 0) == 1;
	...
	public static void SetFirebaseDebug()
	{
		var debugEnabled = PlayerPrefs.GetInt("FirebaseDebugEnabled", 0) == 1;
		PlayerPrefs.SetString("/google/firebase/debug_mode", debugEnabled ? "True" : "False");
		PlayerPrefs.SetString("/google/measurement/debug_mode", debugEnabled ? "True" : "False");
	}
	...
	void Start()
	{
		...
		ToggleDebugMode(isFirebaseDebugActive);
	...
	void DrawInfo()
	{
		...
		GUILayout.Space(size.x);
		float _size = GUILayout.HorizontalSlider(size.x, 32, 64, sliderBackStyle, sliderThumbStyle, GUILayout.Width(Screen.width * 0.5f));
		if (size.x != _size) {
			size.x = size.y = _size;
			initializeStyle();
		}
	...
	void DrawToolBar()
	{
		...
		toolBarRect.y = Screen.safeArea.y;
		...
		GUI.skin = toolbarScrollerSkin;
		Vector2 drag = getDrag();
		if ((drag.x != 0) && (downPos != Vector2.zero) && (downPos.y > Screen.height - Screen.safeArea.y - size.y * 2f)) {
	...
	{
#if MINIIT_ADS
		if (GUILayout.Button(adsContent, barStyle, GUILayout.Width(size.x * 2), GUILayout.Height(size.y * 2)))
		{
#if MINIIT_IRONSOURCE
			IronSource.Agent.launchTestSuite();
#elif MINIIT_APPLOVIN
	...
	private void DrawFirebaseDebugButton()
	{
		if(GUILayout.Button(firebaseContent, isFirebaseDebugActive ? buttonActiveStyle : barStyle,
			   GUILayout.Width(size.x * 2), GUILayout.Height(size.y * 2)))
		{
			isFirebaseDebugActive = !isFirebaseDebugActive;
			ToggleDebugMode(isFirebaseDebugActive);
		}
	}

	private void ToggleDebugMode(bool active)
	{
#if FIREBASE_DEBUG
		PlayerPrefs.SetInt("FirebaseDebugEnabled", active ? 1 : 0);

		Debug.Log(active
			? "<color=green>FIREBASE Debug mode enabled</color>"
			: "<color=red>FIREBASE Debug mode disabled</color>");
#endif
	}
	...
	public void OnGUIDraw()
	{
		...
		logsRect.y = Screen.safeArea.y + size.y * 2f;
		logsRect.width = Screen.safeArea.width;
		logsRect.height = Screen.safeArea.height * 0.75f - size.y * 2f;

		stackRectTopLeft.x = Screen.safeArea.x;
		stackRect.x = Screen.safeArea.x;
		...
		detailRect.x = Screen.safeArea.x;
